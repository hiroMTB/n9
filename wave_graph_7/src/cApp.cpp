...
class cApp : public AppNative {
  public:
    ...
    // A
    // 4:37:05 - 4:43:21 center fix
    
    // B
    // 4:44:15 - 4:44:22 center move
    
    // C
    //
    unsigned int startFrame = 6845;
    unsigned int frame = startFrame;
    unsigned int endFrame = 7306;
...
void cApp::setup(){
    ...
    mExp.setup( wW, wH, startFrame, endFrame, GL_RGB, mt::getRenderPath(), 0, "wg6_");
    mPln.setOctaves(4);
...
void cApp::update(){
    ...
    pivot.push_back( axis[4].pos );
    pivot.push_back( axis[5].pos );
    ...
    for ( int w=0; w<wave.size(); w++ ) {
        for ( int c=0; c<wave[w].nCh; c++ ) {
            ...
            int s = 0;
            vboWave[vboIndex].addPos( Vec3f( pivot[0].x, pivot[0].y, 0) );
            vboWave[vboIndex].addCol( ColorAf(0,0,0,0.3) );
            ...
            while( 1 ){
                ...
                float addx = xscale*(aa);
                addx = MAX(addx,1);
                float finalx = pastx + addx;
                pastx = finalx;
...
//                }
            ...
            vboWave[vboIndex].addPos( Vec3f( pivot[1].x, pivot[1].y, 0) );
            vboWave[vboIndex].addCol( ColorAf(0,0,0,0.3) );
    ...
    parallel_for( blocked_range<size_t>(0, 50000),
        ...
        for( int i=r.begin(); i!=r.end(); ++i){
            ...
            if( 30<dist && dist<40.0){
                lpos.push_back( p1 );
                lpos.push_back( p2 );
                lcol.push_back( ColorAf(0,0,0,0.1) );
                lcol.push_back( ColorAf(0,0,0,0.1) );
            }
        }
